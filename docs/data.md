...
Where does all the chosen data come from and how and why?

JLPT resources are primarily found from [Jonathan Waller‘s JLPT list](https://www.tanos.co.uk/jlpt/). In particular, the .csv files are taken from https://github.com/stephenmk/yomitan-jlpt-vocab/tree/main/original_data who add jmdict id values to each entry, and license it under CC-BY-SA-4.0. This resource is used for the online dictionary [Jisho](https://jisho.org) as well as derivatives such as [JLPT Sensei](https://jlptsensei.com/). It is something of a defacto JLPT list, even though it hasn't been updated in over a decade. Many sources either use the words contained or (without specifying) use the majority of the words with a few added or taken away without explanation. I find the list to be a little outdated for general vocabulary (language changes over years and decades), but still a great starting source for the JLPT. I believe this vocab list to be rather good for anyone looking to learn Japanese, not just for passing the test, thanks to it's nice structure going from easy to hard in distinct steps.
...
Sentences are taken from [Tatoeba](https://tatoeba.org) the amazing free resource of crowd-sourced sentences and translations. In particular, the Transcriptions (Japanese) download and the 
